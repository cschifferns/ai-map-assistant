...
const AgentWorkspace = Annotation.Root({
  ...
  mapView: Annotation<MapView | null>({
...
// No tools needed for a domain knowledge agent — the LLM's
// training + system prompt carries the expertise.
// Add tools here later (e.g. BLM PLSS lookup, county recorder
// search, CRTN station query) as the POC evolves.
// ---------------------------------------------------------------
function buildGraph() {
  const llm = getLLM();
...
// ---------------------------------------------------------------
export function createLandSurveyAgent(view: MapView): AgentRegistration {
  return {
    ...
    initialState: {
      mapView: view,
    } as Partial<typeof AgentWorkspace.State>,
  };
}
